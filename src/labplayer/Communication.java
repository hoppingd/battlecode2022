...
// [15] delayModifier: modifier pow - 1
...
// [28] Crunch index = idx
// [29-42] Enemy logs and ids = yyyy yyxx xxxx / id (6 bits)
// [62] Transmute flag: 0|1
// [63] Mutator flag: 0|1
package labplayer;
...
public class Communication {
    ...
    boolean shouldTransmute() {
        boolean b = false;
        try {
            b = rc.readSharedArray(62) == 0;
        } catch (Throwable t) {
            t.printStackTrace();
        }
        return b;
    ...
    boolean getMutatorFlag() {
        int flag = 0;
        try {
            flag = rc.readSharedArray(63);
        } catch (Throwable t) {
            t.printStackTrace();
        }
        return flag == 1;
    ...
    int getDelayModifier() {
        int modifier = 0;
        try {
            modifier = rc.readSharedArray(15);
        } catch (Throwable t) {
            t.printStackTrace();
        }
        return (int)Math.pow(2, modifier + 1);
    }

    void incDelayModifier() {
        try {
            int modifier = rc.readSharedArray(15);
            rc.writeSharedArray(15, modifier + 1);
        } catch (Throwable t) {
            t.printStackTrace();
        }
    }
